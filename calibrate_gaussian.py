...
for amp in amplitudes:
    gauss_pulse = pulse_lib.gaussian(
        duration=duration,
        amp=amp,
        sigma=sigma,
    ...
    pi_schedule += Play(gauss_pulse, drive_chan)
...
fit_crop_parameter = int(0.5 * len(amplitudes))
...
if os.path.isfile(params_file):
    ...
    param_df.to_csv(params_file)
else:
    param_series.to_frame().T.to_csv(params_file)
