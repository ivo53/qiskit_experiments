...
def step_unitary(H: jnp.ndarray, dt: float) -> jnp.ndarray:
    """Closed-form exp(-i H dt) for 2×2 H = (1/2) h·σ.
    U = cos(θ) I - i sin(θ) (H/||H||) with θ = ||H|| dt.
    Numerically stable for small ||H|| by using series fallback.
    """
    # Extract vector components via traces: H = 0.5*(hx σx + hy σy + hz σz)
    hx = jnp.real(jnp.trace(H @ SX))
    hy = jnp.real(jnp.trace(H @ SY))
    hz = jnp.real(jnp.trace(H @ SZ))
    hnorm = 0.5 * jnp.sqrt(hx * hx + hy * hy + hz * hz)  # because of 0.5 in H definition
...
def build_loss(cfg: Config, t, dt, delta0_grid):
    ...
    # Center index around Δ0≈0 (grid is symmetric/odd-length)
    ...
    def loss_fn(vars: Variables) -> float:
        ...
        smooth = cfg.lam_smooth * (jnp.sum(dOm * dOm) + jnp.sum(dPh * dPh))
