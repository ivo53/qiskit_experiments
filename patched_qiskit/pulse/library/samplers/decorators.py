...
def custom_midpoint(continuous_pulse: Callable) -> Callable:
    r"""Custom midpoint sampling strategy decorator.

    For `start`, 'end', return:
        $$\{f(t+0.5) \in \mathbb{C} | t \in \mathbb{Z} \wedge  \texttt{start}<=t<\texttt{end}\}$$

    Args:
        continuous_pulse: To sample.
    """
    return sampler(strategies.custom_midpoint_sample)(continuous_pulse)
