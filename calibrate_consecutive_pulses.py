...
for amp in amplitudes:
    ...
    lor_num_pulses = 10
    lor_dur_dt = lor_num_pulses * 2576
    lor_B = 0
    G = 2000
    lor_t_dt = np.arange(lor_dur_dt)
    lor_amps = G * amp * G / ((lor_t_dt - lor_dur_dt / 2) ** 2 + G ** 2) + lor_B
    lor = [None for _ in range(lor_num_pulses)]
    for i in range(lor_num_pulses):
        amps_from = int(i * lor_dur_dt / lor_num_pulses)
        amps_to = int((i + 1) * lor_dur_dt / lor_num_pulses)
        lor[i] = pulse_lib.Waveform(
            samples=lor_amps[amps_from:amps_to], 
            name=f"lorentzian pulse part {i}"
        )
    ...
    lor2_amps = G2 * amp * G2 / ((lor2_t_dt - lor2_dur_dt / 2) ** 2 + G2 ** 2) + lor2_B
    ...
    for p in pulses[pulse_type]:
        pi_schedule += Play(p, drive_chan)
...
rabi_fit_params, rabi_y_fit = fit_function(
    ...
    [-2., 20, -8.5] # initial parameters for curve_fit
)
