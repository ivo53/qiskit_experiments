...
amplitude_multipliers = np.arange(1.04, 1.0801, 0.0001)
...
for j, a in enumerate(amplitude_multipliers):
    ...
    dd_schedule = pulse.Schedule(name=f"{num_dd_blocks} pi pulses")
...
jobs = job_manager.run(dd_schedules, backend=backend, name="Pi Area Fine Calibration", shots=num_shots_per_exp)
results = jobs.results()
transition_probability = []
for i in range(len(amplitude_multipliers)):
...
plt.savefig(os.path.join(save_dir, date.strftime("%H%M%S") + f"_duration-{pi_pulse_duration}_amp_variation.png"))
plt.show()
