...
dur_dt = 50 * 16 #1117 * 16 #525 * 16 #644 * 16 #483 * 16 #5152
...
frequency_span_Hz = 50 * MHz #5 * MHz #if cut_param < 1 els e 1.25 * MHz
...
amp = Parameter('amp')
with pulse.build(backend=backend, default_alignment='sequential', name="rect_2d") as sched:
...
rect = Gate("rect", 1, [amp, freq_off])
base_circ = QuantumCircuit(5, 1)
base_circ.append(rect, [qubit])
base_circ.measure(qubit, 0)
base_circ.add_calibration(rect, (qubit,), sched, [amp, freq_off])
circs = [
...
for i, am in enumerate(amplitudes):
    ...
    plt.title(f"Rabi Freq Offset - Amplitude {am.round(3)}")
