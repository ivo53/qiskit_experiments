...
## Demkov
demkov_dur_dt = 2576
demkov_k = 12.75810371
demkov_t_dt = np.arange(demkov_dur_dt)
demkov_A = np.pi / demkov_k
demkov_B = 250
demkov_amps = demkov_A * np.exp(-np.abs(demkov_t_dt - demkov_dur_dt / 2) / demkov_B)
...
df.to_csv(
    os.path.join(
        data_folder, 
        date.strftime("%H%M%S") + f"_{pulse_type}_calibration.csv"
    ),
    index=False
)
plt.figure(1)
...
plt.savefig(os.path.join(save_dir, date.strftime("%H%M%S") + f"_{pulse_type}_frequency_sweep.png"))
# plt.show()
