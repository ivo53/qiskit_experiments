...
pi_amp = np.pi / k
sigma = 0.05
sigma = get_closest_multiple_of_16(sigma * us / dt)
duration = get_closest_multiple_of_16(8 * sigma * us / dt)
...
for j, area in enumerate(areas):
    ...
    if duration == 0 or amp == 0:
        pb_schedule = pulse.Schedule(name=f"Gaussian Power Broadening {j}")
    ...
    plt.title(f"Gaussian Pulse with Rabi Amp {amp}")
    plt.savefig(
        os.path.join(
            save_dir, 
            date.strftime("%H%M%S"), 
...
plt.title("Gaussian Power Broadening")
bar = plt.colorbar(im)
