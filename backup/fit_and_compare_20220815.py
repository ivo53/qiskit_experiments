...
fit_params, y_fit = fit_function(
    ...
    [1, 0, 15, 15, 0], # initial parameters for curve_fit
    [0, -10, -0.05, 0, 0], # upper bounds on parameters
    [1e8, 10, 100, 100, 1] # lower bounds on parameters
...
ax0.plot(extended_freq, compare_extended_y_fit, color='blue')
...
ax0.set_title(f"Fitted {pulse_type.capitalize()} Frequency Curve (Area {area.capitalize()})", fontsize=22)
...
limit_num = np.ceil(np.amax([max(compare_y_fit - vals), np.abs(min(compare_y_fit - vals))]) / 0.05) * 0.05
...
# ax2.scatter(detuning, compare_y_fit - vals, color="blue", marker="*")
...
ax2.errorbar(detuning, compare_y_fit - vals, yerr=err * np.ones(detuning.shape), fmt="+", color="b")
if limit_num > 0.1:
