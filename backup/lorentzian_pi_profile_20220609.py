...
dur_dt = 2256 #1117 * 16 #525 * 16 #644 * 16 #483 * 16 #5152

# ## set by cut parameter
# cut_param = 50
...
a_max = 0.45
a_step = np.round(a_max / 3, 3)

frequency_span_Hz = 20 * MHz #5 * MHz #if cut_param < 1 else 1.25 * MHz
...
dur_pi, k, l, p = 2256, np.float64(1.), np.float64(96.41208781), np.float64(0.64105608)
...
while np.abs(S - S_pi) > 1e-9:
    G = G - (S - S_pi) * lamb
    S = 2 * amp_pi * G * np.arctan(dur_dt/(2*G))
print("G =", G)
cut_param = 1 / (1 + (dur_dt / (2*G))**2)

amp_half = S_half * amp_pi / S_pi

idx_half = np.argmin(
    np.abs(amplitudes - amp_half)
)
idx_pi = np.argmin(
    np.abs(amplitudes - amp_pi)
)
assert idx_half != idx_pi
amplitudes[idx_half] = amp_half
amplitudes[idx_pi] = amp_pi

print(f"The gamma factor is G = {G} and cut param is {cut_param}")#, \
# compared to G_02 = {G_02} at cut param 0.2.")
print(f"The sweep will go from {frequency_min / GHz} GHz to {frequency_max / GHz} GHz \
in steps of {frequency_step_Hz / MHz} MHz.")
print(f"The amplitude will go from {amplitudes[0]} to {amplitudes[-1]} in approx steps of {a_step}:")
print(amplitudes)
...
for i, am in enumerate(amplitudes):
    ...
    if i == idx_half:
        plt.savefig(os.path.join(folder_name, f"lor_amp-{am.round(3)}_half-pi.png").replace("\\","/"))
    elif i == idx_pi:
        plt.savefig(os.path.join(folder_name, f"lor_amp-{am.round(3)}_pi.png").replace("\\","/"))
    else:
