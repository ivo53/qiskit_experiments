...
def sine(times: np.ndarray, amp: complex, freq: float, phase: float = 0) -> np.ndarray:
    """Continuous cosine wave.
    ...
    Args:
        ...
        phase: Pulse phase.
    """
    return amp * np.sin(np.pi * freq * times + phase).astype(np.complex_)


def sine_square(times: np.ndarray, amp: complex, freq: float, phase: float = 0) -> np.ndarray:
    """Continuous cosine wave.
    ...
    Args:
        ...
        phase: Pulse phase.
    """
    return amp * (np.sin(np.pi * freq * times + phase) ** 2).astype(np.complex_)


def sine_cube(times: np.ndarray, amp: complex, freq: float, phase: float = 0) -> np.ndarray:
    """Continuous sine_cube wave.
    ...
    Args:
        ...
        phase: Pulse phase.
    """
    return amp * (np.sin(np.pi * freq * times + phase) ** 3).astype(np.complex_)


def sine_fourth_power(times: np.ndarray, amp: complex, freq: float, phase: float = 0) -> np.ndarray:
    """Continuous sine_fourth_power wave.
    ...
    Args:
        ...
        phase: Pulse phase.
    """
    return amp * (np.sin(np.pi * freq * times + phase) ** 4).astype(np.complex_)


def sine_fifth_power(times: np.ndarray, amp: complex, freq: float, phase: float = 0) -> np.ndarray:
    """Continuous sine_fifth_power wave.
    ...
    Args:
        ...
        phase: Pulse phase.
    """
    return amp * (np.sin(np.pi * freq * times + phase) ** 5).astype(np.complex_)
...
def _fix_sech_square_width(
    ...
    r"""Enforce that the supplied sech^2 pulse is zeroed at a specific width.
    ...
    center: Center (mean) of pulse.
    sigma: Standard deviation of pulse.
    zeroed_width: Subtract baseline from sech^2 pulses to make sure
        $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
        large discontinuities at the start of a sech^2 pulse. If unsupplied,
        defaults to $2*(center + 1)$ such that $\Omega_g(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
    rescale_amp: If True the pulse will be rescaled so that $\Omega_g(center)=amp$.
    ret_scale_factor: Return amplitude scale factor.
    """
    ...
    zero_offset = sech_square(np.array([zeroed_width / 2]), amp, 0, sigma)
    sech_square_samples -= zero_offset
    amp_scale_factor = 1.0
    if rescale_amp:
        amp_scale_factor = amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        sech_square_samples *= amp_scale_factor

    if ret_scale_factor:
        return sech_square_samples, amp_scale_factor
    return sech_square_samples
...
def sech_square(
    ...
    r"""Continuous unnormalized sech square pulse.
    ...
    times = np.asarray(times, dtype=np.complex_)
    x = (times - center) / sigma
    sech_square_out = amp * (sech_fn(x).astype(np.complex_)) ** 2

    if zeroed_width is not None:
        sech_square_out = _fix_sech_square_width(
            sech_square_out,
            amp=amp,
            center=center,
            sigma=sigma,
    ...
    if ret_x:
        return sech_square_out, x
    return sech_square_out
...
def _fix_lorentz_width(
    ...
    r"""Enforce that the supplied lorentzian pulse is zeroed at a specific width.

    This is achieved by subtracting $\Omega_l(center \pm zeroed_width/2)$ from all samples.

    amp: Pulse amplitude at `center`.
    center: Center (mean) of pulse.
    gamma: Standard deviation of pulse.
    zeroed_width: Subtract baseline from lorentzian pulses to make sure
        $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
        large discontinuities at the start of a lorentzian pulse. If unsupplied,
        defaults to $2*(center + 1)$ such that $\Omega_l(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
    rescale_amp: If True the pulse will be rescaled so that $\Omega_g(center)=amp$.
    ret_scale_factor: Return amplitude scale factor.
    """
    if zeroed_width is None:
    ...
    zero_offset = lorentzian(np.array([zeroed_width / 2]), amp, 0, gamma)
    lorentz_samples -= zero_offset
    amp_scale_factor = 1.0
    if rescale_amp:
        amp_scale_factor = amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        lorentz_samples *= amp_scale_factor

    if ret_scale_factor:
        return lorentz_samples, amp_scale_factor
    return lorentz_samples
...
def lorentzian(
    ...
    r"""Continuous unnormalized lorentzian pulse.
    ...
    Args:
        ...
        amp: Pulse amplitude at `center`. If `zeroed_width` is set pulse amplitude at center
            ...
            amplitude will be `amp`.
        center: Center (mean) of pulse.
        gamma: Width of pulse.
        zeroed_width: Subtract baseline from lorentzian pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
            large discontinuities at the start of a lorentzian pulse.
        rescale_amp: If `zeroed_width` is not `None` and `rescale_amp=True` the pulse will
            be rescaled so that $\Omega_g(center)=amp$.
        ret_x: Return centered and standard deviation normalized pulse location.
               $x=(times-center)/gamma.
    """
    times = np.asarray(times, dtype=np.complex_)
    x = (times - center) / gamma
    lor = amp * (1. / ((x ** 2) + 1.)).astype(np.complex_)

    if zeroed_width is not None:
        lor = _fix_lorentz_width(
            lor,
            amp=amp,
            center=center,
            gamma=gamma,
    ...
    if ret_x:
        return lor, x
    return lor
def _fix_lorentz_square_width(
    lorentz_square_samples,
    amp: float,
    center: float,
    gamma: float,
    zeroed_width: Optional[float] = None,
    ...
    r"""Enforce that the supplied squared lorentzian pulse is zeroed at a specific width.
    ...
    center: Center (mean) of pulse.
    gamma: Standard deviation of pulse.
    zeroed_width: Subtract baseline from squared lorentzian pulses to make sure
        $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
        large discontinuities at the start of a squared lorentzian pulse. If unsupplied,
        defaults to $2*(center + 1)$ such that $\Omega_l(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
    rescale_amp: If True the pulse will be rescaled so that $\Omega_g(center)=amp$.
    ret_scale_factor: Return amplitude scale factor.
    """
    ...
    zero_offset = lorentzian_square(np.array([zeroed_width / 2]), amp, 0, gamma)
    lorentz_square_samples -= zero_offset
    amp_scale_factor = 1.0
    if rescale_amp:
        amp_scale_factor = amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        lorentz_square_samples *= amp_scale_factor

    if ret_scale_factor:
        return lorentz_square_samples, amp_scale_factor
    return lorentz_square_samples


def lorentzian_square(
    times: np.ndarray,
    amp: complex,
    center: float,
    gamma: float,
    ...
    r"""Continuous unnormalized squared lorentzian pulse.
    ...
    Args:
        ...
        zeroed_width: Subtract baseline from squared lorentzian pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
            large discontinuities at the start of a squared lorentzian pulse.
        rescale_amp: If `zeroed_width` is not `None` and `rescale_amp=True` the pulse will
            be rescaled so that $\Omega_g(center)=amp$.
        ret_x: Return centered and standard deviation normalized pulse location.
               $x=(times-center)/sigma.
    ...
    times = np.asarray(times, dtype=np.complex_)
    x = (times - center) / gamma
    lor_sq = amp * ((1. / ((x ** 2) + 1.)) ** 2).astype(np.complex_)

    if zeroed_width is not None:
        lor_sq = _fix_lorentz_square_width(
            lor_sq,
            amp=amp,
            center=center,
            gamma=gamma,
...
def _fix_lorentz_cube_width(
    lorentz_cube_samples,
    amp: float,
    center: float,
    gamma: float,
    zeroed_width: Optional[float] = None,
    ...
    r"""Enforce that the supplied cubed lorentzian pulse is zeroed at a specific width.
    ...
    center: Center (mean) of pulse.
    gamma: Standard deviation of pulse.
    zeroed_width: Subtract baseline from cubed lorentzian pulses to make sure
        $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
        large discontinuities at the start of a cubed lorentzian pulse. If unsupplied,
        defaults to $2*(center + 1)$ such that $\Omega_l(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
    rescale_amp: If True the pulse will be rescaled so that $\Omega_g(center)=amp$.
    ret_scale_factor: Return amplitude scale factor.
    """
    ...
    zero_offset = lorentzian_cube(np.array([zeroed_width / 2]), amp, 0, gamma)
    lorentz_cube_samples -= zero_offset
    amp_scale_factor = 1.0
    if rescale_amp:
        amp_scale_factor = amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        lorentz_cube_samples *= amp_scale_factor

    if ret_scale_factor:
        return lorentz_cube_samples, amp_scale_factor
    return lorentz_cube_samples
...
def lorentzian_cube(
    ...
    r"""Continuous unnormalized cubed lorentzian pulse.
    ...
    Args:
        ...
        zeroed_width: Subtract baseline from cubed lorentzian pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
            large discontinuities at the start of a cubed lorentzian pulse.
        rescale_amp: If `zeroed_width` is not `None` and `rescale_amp=True` the pulse will
            be rescaled so that $\Omega_g(center)=amp$.
        ret_x: Return centered and standard deviation normalized pulse location.
               $x=(times-center)/sigma.
    ...
    times = np.asarray(times, dtype=np.complex_)
    x = (times - center) / gamma
    lor_cube = amp * ((1. / ((x ** 2) + 1.)) ** 3).astype(np.complex_)

    if zeroed_width is not None:
        lor_cube = _fix_lorentz_cube_width(
            lor_cube,
            amp=amp,
            center=center,
            gamma=gamma,
...
def _fix_demkov_width(
    ...
    r"""Enforce that the supplied Demkov pulse is zeroed at a specific width.
    ...
    center: Center (mean) of pulse.
    sigma: Standard deviation of pulse.
    zeroed_width: Subtract baseline from Demkov pulses to make sure
        $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
        large discontinuities at the start of a Demkov pulse. If unsupplied,
        defaults to $2*(center + 1)$ such that $\Omega_l(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
    rescale_amp: If True the pulse will be rescaled so that $\Omega_g(center)=amp$.
    ret_scale_factor: Return amplitude scale factor.
    """
    ...
    zero_offset = demkov(np.array([zeroed_width / 2]), amp, 0, sigma)
    demkov_samples -= zero_offset
    amp_scale_factor = 1.0
    if rescale_amp:
        amp_scale_factor = amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        demkov_samples *= amp_scale_factor

    if ret_scale_factor:
        return demkov_samples, amp_scale_factor
    return demkov_samples
...
def demkov(
    ...
    r"""Continuous unnormalized Demkov pulse.
    ...
    Args:
        ...
        zeroed_width: Subtract baseline from Demkov pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is used to avoid
            large discontinuities at the start of a Demkov pulse.
        rescale_amp: If `zeroed_width` is not `None` and `rescale_amp=True` the pulse will
            be rescaled so that $\Omega_g(center)=amp$.
        ret_x: Return centered and standard deviation normalized pulse location.
               $x=(times-center)/sigma.
    ...
    times = np.asarray(times, dtype=np.complex_)
    x = (times - center) / sigma
    _demkov = amp * (np.exp(-np.abs(x))).astype(np.complex_)

    if zeroed_width is not None:
        _demkov = _fix_demkov_width(
            _demkov,
            amp=amp,
            center=center,
            gamma=sigma,
