...
_sampled_sine_pulse = samplers.midpoint(continuous.sine)
...
def sine(
    ...
    return _sampled_sine_pulse(duration, amp, freq, phase=phase, name=name)


_sampled_sine_square_pulse = samplers.midpoint(continuous.sine_square)
...
def sine_square(
    ...
    return _sampled_sine_square_pulse(duration, amp, freq, phase=phase, name=name)


_sampled_sine_cube_pulse = samplers.midpoint(continuous.sine_cube)
...
def sine_cube(
    ...
    return _sampled_sine_cube_pulse(duration, amp, freq, phase=phase, name=name)


_sampled_sine_fourth_power_pulse = samplers.midpoint(continuous.sine_fourth_power)


def sine_fourth_power(
    duration: int, amp: complex, freq: float = None, phase: float = 0, name: Optional[str] = None
) -> Waveform:
    r"""Generates sine wave :class:`~qiskit.pulse.library.Waveform`.
    ...
    return _sampled_sine_fourth_power_pulse(duration, amp, freq, phase=phase, name=name)


_sampled_sine_fifth_power_pulse = samplers.midpoint(continuous.sine_fifth_power)


def sine_fifth_power(
    duration: int, amp: complex, freq: float = None, phase: float = 0, name: Optional[str] = None
) -> Waveform:
    r"""Generates sine^5 wave :class:`~qiskit.pulse.library.Waveform`.
    ...
    return _sampled_sine_fifth_power_pulse(duration, amp, freq, phase=phase, name=name)
...
_sampled_sech_square_pulse = samplers.midpoint(continuous.sech_square)


def sech_square(
    duration: int, amp: complex, sigma: float, name: str = None, zero_ends: bool = True
) -> Waveform:
    r"""Generates unnormalized sech^2 :class:`~qiskit.pulse.library.Waveform`.
    ...
    .. math::
        ...
        f(x) = A\text{sech}^2\left(\frac{x-\mu}{\sigma} \right)
    ...
    zeroed_width = duration + 2 if zero_ends else None
    rescale_amp = bool(zero_ends)
    return _sampled_sech_square_pulse(
        duration, amp, center, sigma, zeroed_width=zeroed_width, rescale_amp=rescale_amp, name=name
    )
...
_sampled_lorentz_pulse = samplers.midpoint(continuous.lorentzian)


def lorentzian(
    duration: int, amp: complex, gamma: float, name: Optional[str] = None, zero_ends: bool = True
) -> Waveform:
    r"""Generates unnormalized lorentzian :class:`~qiskit.pulse.library.Waveform`.

    For :math:`A=` ``amp`` and :math:`\Gamma=` ``gamma``, applies the ``midpoint`` sampling strategy
    to generate a discrete pulse sampled from the continuous function:

    .. math::
        f(x) = A\frac{\Gamma^2}{\left(x - \mu\right)^2 + \Gamma^2},
    ...
    zeroed_width = duration + 2 if zero_ends else None
    rescale_amp = bool(zero_ends)
    return _sampled_lorentz_pulse(
        duration, amp, center, gamma, zeroed_width=zeroed_width, rescale_amp=rescale_amp, name=name
    )


_custom_sampled_lorentz_pulse = samplers.custom_midpoint(continuous.lorentzian)


def lorentzian_fragment(
    duration: int, start: int, end: int, center: float, amp: complex, gamma: float, name: Optional[str] = None, zero_ends: bool = False
) -> Waveform:
    r"""Generates unnormalized lorentzian :class:`~qiskit.pulse.library.Waveform`.
    ...
    if duration != end - start:
        print("Duration must be equal to end - start!")
    zeroed_width = start - end + 2 if zero_ends else None
    rescale_amp = bool(zero_ends)
    return _custom_sampled_lorentz_pulse(
        duration, start, end, amp, center, gamma, zeroed_width=zeroed_width, rescale_amp=rescale_amp, name=name
    )


_sampled_lorentz_square_pulse = samplers.midpoint(continuous.lorentzian_square)


def lorentzian_square(
    duration: int, amp: complex, gamma: float, name: Optional[str] = None, zero_ends: bool = True
) -> Waveform:
    r"""Generates unnormalized squared lorentzian :class:`~qiskit.pulse.library.Waveform`.
    ...
    zeroed_width = duration + 2 if zero_ends else None
    rescale_amp = bool(zero_ends)
    return _sampled_lorentz_square_pulse(
        duration, amp, center, gamma, zeroed_width=zeroed_width, rescale_amp=rescale_amp, name=name
    )


_sampled_lorentz_cube_pulse = samplers.midpoint(continuous.lorentzian_cube)


def lorentzian_cube(
    duration: int, amp: complex, gamma: float, name: Optional[str] = None, zero_ends: bool = True
) -> Waveform:
    r"""Generates unnormalized cubed lorentzian :class:`~qiskit.pulse.library.Waveform`.
    ...
    Args:
        duration: Duration of pulse. Must be greater than zero.
        amp: Pulse amplitude at ``duration/2``.
        gamma: Width (standard deviation) of pulse.
        name: Name of pulse.
        zero_ends: If True, zero ends at ``x = -1, x = duration + 1``, but rescale to preserve amp.
    """
    center = duration / 2
    zeroed_width = duration + 2 if zero_ends else None
    rescale_amp = bool(zero_ends)
    return _sampled_lorentz_cube_pulse(
        duration, amp, center, gamma, zeroed_width=zeroed_width, rescale_amp=rescale_amp, name=name
    )


_sampled_demkov_pulse = samplers.midpoint(continuous.demkov)

def demkov(
    duration: int, amp: complex, sigma: float, name: Optional[str] = None, zero_ends: bool = True
) -> Waveform:
    r"""Generates unnormalized Demkov :class:`~qiskit.pulse.library.Waveform`.

    For :math:`A=` ``amp`` and :math:`\sigma=` ``sigma``, applies the ``midpoint`` sampling strategy
    to generate a discrete pulse sampled from the continuous function:

    .. math::
        f(x) = A\exp{-\frac{\abs{x-\mu}}{\{sigma}}},
