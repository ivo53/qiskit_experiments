...
def add_circ(backend, drive_chan, pulse_type, amp, duration, sigma, remove_bg, chirp, tau, freq, qubit=0):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-rC", "--resolution_C", default=50, type=int,
    ...
    parser.add_argument("-tau", "--tau", default=10, type=float,
        help="The tau parameter for the Allan-Eberly function.")
    parser.add_argument("-sp", "--chirp_span", default=18, type=float,
        help="Chirp span in MHz units.")
    ...
    chirp_center_Hz = 0
    chirp_min = chirp_center_Hz - chirp_span_Hz / 2
    chirp_max = chirp_center_Hz + chirp_span_Hz / 2

    # Construct an np array of the chirps for our experiment
    ...
    circs = [add_circ(
        ...
        sigma, remove_bg, c, tau, center_frequency_Hz, qubit
        ) for a in amplitudes for c in chirps_Hz]
    ...
    transition_probability = np.array(transition_probability).reshape(len(amplitudes), len(chirps_GHz))
