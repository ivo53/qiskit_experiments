...
freq = Parameter('freq')
amp = Parameter('amp')
with pulse.build(backend=backend, default_alignment='sequential', name="sech_2d") as sched:
...
for i, am in enumerate(amplitudes):
    ...
    plt.title(f"Sech Freq Offset - Amplitude {am.round(3)}")
    plt.savefig(os.path.join(folder_name, f"sech_amp-{am.round(3)}.png"))
    plt.close()
...
plt.savefig(
    os.path.join(
        save_dir,
        f"{date.strftime('%H%M%S')}_sech_pwr_nrw_duration-{dur_dt}dt_sigma-{sigma}.png"
    ).replace("\\","/")
)
plt.show()