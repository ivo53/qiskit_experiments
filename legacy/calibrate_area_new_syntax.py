...
with pulse.build(backend=backend, default_alignment='sequential', name="calibrate_area") as sched:
    ...
    print(f"G is {G} and cut_param is {cutparam}%.")
    pulse.play(
...
with open(os.path.join(data_folder, f"{dur_dt}dt_cutparam-{cutparam}%_G-{G}_amps.pkl").replace("\\","/"), 'wb') as f1:
    pickle.dump(amplitudes, f1)
with open(os.path.join(data_folder, f"{dur_dt}dt_cutparam-{cutparam}%_G-{G}_tr_prob.pkl").replace("\\","/"), 'wb') as f2:
...
rabi_fit_params, _ = fit_function(
    ...
    [-0.47273362, 43.61932758, 0.18867079, 0.47747625]
...
pi5_amp = - np.log(1 - 5 * np.pi / l) / p
print("pi5_amp =", pi5_amp)
