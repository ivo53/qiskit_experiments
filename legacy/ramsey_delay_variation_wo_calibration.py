...
delays = np.linspace(0, 10 * duration, num_ramsey_delay_exp)
schedules = []
for t in delays:
    ramsey_schedule = pulse.Schedule(name=f"Ramsey fringe experiment")
    ramsey_schedule += Play(half_pi_pulse, drive_chan)
    ramsey_schedule += Delay(get_closest_multiple_of_16(t), drive_chan)
    ramsey_schedule += Play(half_pi_pulse, drive_chan)
...
ramsey_qobj = assemble(
    ...
    schedule_los=[{drive_chan: rough_qubit_frequency}] * num_ramsey_delay_exp
)
...
plt.figure(5)
plt.scatter(delays, np.real(ramsey_values), color='black') # plot real part of sweep values
