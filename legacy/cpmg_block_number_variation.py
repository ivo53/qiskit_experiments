...
calib_dir = os.path.join(file_dir, "plots", "cpmg")
...
num_dd_blocks = np.arange(0, max_num_dd_blocks + 1, 4)
...
amplitude_multipliers = np.arange(1.05, 1.061, 0.0025)
...
for j, a in enumerate(amplitude_multipliers):
    ...
    plt.xlabel("# CPMG blocks")
    plt.ylabel("Excited state population")

    plt.savefig(os.path.join(save_dir, date.strftime("%H%M%S"), f"_duration-{pi_pulse_duration}_amp-{a}_block_variation.png"))
    plt.close()
...
plt.title("|1> population for varied CPMG blocks numbers and amplitudes")
bar = plt.colorbar(im)
