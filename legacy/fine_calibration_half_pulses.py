...
calib_dir = os.path.join(file_dir, "plots", "half_pulses_amplitude_variation")
...
## set run variables
max_num_dd_blocks = 150
num_dd_blocks = np.arange(3, max_num_dd_blocks + 1, 4)
...
amplitude_multipliers = np.arange(.9999, 1.001101, 0.0001)
# amplitude_multipliers = np.arange(1.054, 1.064001, 0.00015)
...
for j, a in enumerate(amplitude_multipliers):
    ...
    for n in num_dd_blocks:
        ...
        dd_schedule = pulse.Schedule(name=f"{n} pi/2 pulses")
    ...
    plt.figure(j)
    plt.scatter(num_dd_blocks, dd_values, color='black')
    plt.xlabel("# Half Pi pulses")
    plt.ylabel("Transition Probability")

    plt.savefig(os.path.join(save_dir, date.strftime("%H%M%S"), f"_duration-{np.round(pi_pulse_duration, 2)}_amp-{np.round(a, 5)}_block_variation.png"))
    plt.close()
...
plt.xlabel("Number of Pi/2 pulses")
plt.ylabel("Amplitude [a.u.]")
plt.title("Transition probability for varied CPMG blocks numbers and amplitudes")
bar = plt.colorbar(im)
bar.set_label('Excited state population')
plt.savefig(os.path.join(save_dir, date.strftime("%H%M%S") + f"_duration-{np.round(pi_pulse_duration, 2)}_block_variation.png"))
plt.show()