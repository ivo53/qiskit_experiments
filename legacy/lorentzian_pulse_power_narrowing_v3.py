...
dur_dt = 1117 * 16 #525 * 16 #644 * 16 #483 * 16 #5152
...
a_max = 0.92
a_step = np.round(a_max / 100, 3)

frequency_span_Hz = 10 * MHz #5 * MHz #if cut_param < 1 els e 1.25 * MHz
frequency_step_Hz = np.round(frequency_span_Hz / 48, 3) #(1/4) * MHz

max_experiments_per_job = 10
...
with pulse.build(backend=backend, default_alignment='sequential', name="sq_2d") as sched:
    ...
    # )
    pulse.play(
        pulse_lib.LorentzianFragment(
            duration=int(base_dur_dt // 3) * 16,
            start=0,
            end=int(base_dur_dt // 3) * 16,
            center=dur_dt/2,
            amp=amp,
            gamma=G,
            name='lor_pulse_1',
    ...
    pulse.play(
        pulse_lib.LorentzianFragment(
            duration=int(base_dur_dt // 3) * 16,
            start=dur_dt - int(base_dur_dt // 3) * 16,
            end=dur_dt,
            center=dur_dt/2,
            amp=amp,
            gamma=G,
